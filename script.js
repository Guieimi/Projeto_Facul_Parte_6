...
function showSlide(index) {
    ...
    // Atualiza o estado das bolinhas de navegação
    dots.forEach((dot, i) => {
...
const slideInterval = setInterval(autoSlide, 4000); // Muda a cada 4 segundos

dots.forEach((dot, i) => {
    dot.addEventListener("click", () => {
        showSlide(i);
        clearInterval(slideInterval);
        slideInterval = setInterval(autoSlide, 4000); // Reinicia o temporizador após clicar em uma bolinha
    });
});
